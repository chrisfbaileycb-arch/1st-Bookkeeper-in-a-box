CREATE FUNCTION commit_staged_transaction(p_stage BIGINT,p_business BIGINT,p_actor TEXT) RETURNS BIGINT LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 INSERT INTO ledger_entries(business_id,entry_date,memo,source_document_id,staged_transaction_id,posted_by,idempotency_key)
 VALUES(p_business,s.transaction_date,s.memo,s.source_document_id,s.id,p_actor,'stage:'||s.id) RETURNING id INTO eid;
 ...
 INSERT INTO audit_events(business_id,actor_id,event_type,entity_type,entity_id,after_value)
 VALUES(p_business,p_actor,'ledger_committed','ledger_entry',eid::text,jsonb_build_object('staged_transaction_id',p_stage,'debits',d,'credits',c));
 RETURN eid;
EXCEPTION WHEN unique_violation THEN
 SELECT id INTO eid FROM ledger_entries WHERE business_id=p_business AND idempotency_key='stage:'||p_stage;
 RETURN eid;
END $$;